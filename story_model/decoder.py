...
import numpy
import warnings
from story_model.search import gen_sample
...
def run_sampler(dec, c, maxlen, beam_width=1, stochastic=False, use_unk=False):
    """
    ...
    sample, score = gen_sample(dec['tparams'], dec['f_init'], dec['f_next'],
                               ...
                               trng=dec['trng'], k=beam_width, maxlen=maxlen, stochastic=stochastic,
                               use_unk=use_unk)
    text = []
    if stochastic:
        sample = [sample]
